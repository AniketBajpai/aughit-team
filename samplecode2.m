...
radius=17 ;width=10;
                                                      % to get the middle two values of x coordintes which will be used in reflection later
leftb=10 ;
...
%while(true)                                    %%aiming bricks one by one , or true!!
...
%end
                       ...
                       %% to be read and only 1st frame 
frames = getdata(vid,1);
 diff_im = imsubtract(frames(:,:,1), rgb2gray(frames));
    diff_im=medfilt2(diff_im,[3 3]);
    diff_im=im2bw(diff_im,0.1);
    diff_im=bwareaopen(diff_im,50);                            
    bw6=bwlabel(diff_im,8);

     
                        %% midPadx and midPady and we have to find that somehow
...
for midx != new_pos
                   ...
                   %% again detecct midPadx and midPady 
...
a.servowrite(servopin,rotation);